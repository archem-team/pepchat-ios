...
class LinkPreviewView: UIView {
    ...
    func configure(with embed: Embed, viewState: ViewState) {
        ...
        case .website(let websiteEmbed):
            hasContent = configureWebsiteEmbed(websiteEmbed)
        case .image(let imageEmbed):
            hasContent = configureImageEmbed(imageEmbed)
        case .video(let videoEmbed):
            hasContent = configureVideoEmbed(videoEmbed)
        case .text(let textEmbed):
            hasContent = configureTextEmbed(textEmbed)
        case .none:
            hasContent = false
        }
        
        // Hide the entire view if there's no meaningful content
        isHidden = !hasContent
    }
    ...
    private func configureWebsiteEmbed(_ websiteEmbed: WebsiteEmbed) -> Bool {
        var hasContent = false
        ...
        if let special = websiteEmbed.special, special != .none {
            if configureSpecialEmbed(websiteEmbed, special: special) {
                hasContent = true
            }
        }
        // Handle video embeds
        else if let video = websiteEmbed.video {
            if configureVideoPreview(video) {
                hasContent = true
            }
        }
        // Handle image embeds
        else if let image = websiteEmbed.image {
            if configureImagePreview(image) {
                hasContent = true
            }
        }
    ...
    private func configureSpecialEmbed(_ websiteEmbed: WebsiteEmbed, special: WebsiteSpecial) -> Bool {
        guard let embedUrl = getSpecialEmbedUrl(websiteEmbed, special: special) else { return false }
    ...
    private func configureImageEmbed(_ imageEmbed: JanuaryImage) -> Bool {
        return configureImagePreview(imageEmbed)
    }
    
    private func configureVideoEmbed(_ videoEmbed: JanuaryVideo) -> Bool {
        return configureVideoPreview(videoEmbed)
    }
    
    private func configureTextEmbed(_ textEmbed: TextEmbed) -> Bool {
        var hasContent = false
